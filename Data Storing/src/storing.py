...
scheme = '''
    ...
    CREATE TABLE IF NOT EXISTS asset_industries(
        asset_id INT,
        industry_id INT,
    ...
    CREATE TABLE IF NOT EXISTS market_cap_yearly_history(
        company_id INT,
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_companies') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_etfs') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_crypto') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_metals') THEN
...
# insert countries, skip if already exist
country_ids = {}
cur.execute('SELECT country_name, country_id FROM countries')
...
print("countries update")

# insert market_assets + companies
ticker_company_ids = {}
cur.execute('''
    SELECT ma.detail_url, c.company_id
    FROM market_assets ma
    JOIN companies c ON ma.asset_id = c.asset_id
''')
...
for c in companies_load:
    ...
    if c['detail_url'] in existing_by_url:
        ticker_company_ids[ticker] = existing_by_url[c['detail_url']]
        continue
...
print("market_assets and companies update")

# insert industries
industry_ids = {}
cur.execute('SELECT industry_name, industry_id FROM industries')
for name, iid in cur.fetchall():
...
for ind in industries_load:
    if ind['name'] not in industry_ids:
        cur.execute(insert_industry, (ind['name'], ind.get('url')))
        industry_ids[ind['name']] = cur.fetchone()[0]
print("industries update")

# insert asset_industries
cur.execute('SELECT company_id, asset_id FROM companies')
company_to_asset = dict(cur.fetchall())

insert_asset_industry = 'INSERT INTO asset_industries(asset_id, industry_id) VALUES (%s, %s) ON CONFLICT DO NOTHING'
...
print("market_cap_snapshots update")
...
print("market_cap_yearly_history update")
